    ...
    describe('Update', function () {
        it('should update values on the Environment', function (done) {
            request(server)
                ...
                .send({"lb_access_logs_s3_bucket_name": "harbor-lb-access-logs-tester"})
                .send({"lb_access_logs_s3_bucket_prefix": `${testShipment.name}-${testEnvironment.name}`})
                .expect('Content-Type', /json/)
                .expect(200, (err, res) => {
                    if (err) {
                        return done(err);
                    ...
                    expect(data.lb_access_logs_s3_bucket_name).to.equal("harbor-lb-access-logs-tester");
                    expect(data.lb_access_logs_s3_bucket_prefix).to.equal("test-shipment-test-env")
                    expect(data.buildToken).to.not.be.null;
