...
function continueLogs(req, res, params, isAuthZed) {
  ...
  promise.then((logs) => {
      let new_logs = logs.map((log) => {
        ...
        if (futurama) {
            // do some magic decrypt on audit logs
        }
...
function getEnvironment(shipmentName, name, isAuthed) {
    ...
    return shipmentPromise.then((shipment) => {
        return environmentPromise.then((environment) => {
            ...
            let sets = [
                {parentId: parentId,  parent: 'parentShipment', name: 'envVars', model: 'EnvVar', encrypt: true},
                {parentId: nParentId, name: 'envVars',    model: 'EnvVar', encrypt: true},
                {parentId: nParentId, name: 'providers',  model: 'Provider',  submodels: [{name: 'envVars', model: 'EnvVar', encrypt: true}]},
                {parentId: nParentId, name: 'containers', model: 'Container',
                    submodels: [{name: 'envVars', model: 'EnvVar', encrypt: true}, {name: 'ports', model: 'Port'}]}
            ],
            callBackCount = 0,
                ...
                sets.forEach(function(s) {
                    ...
                    getAThing(s, isAuthed, (err, sub) => {
                       if (err) {
                           ...
                           callBackCount--;
                           if (callBackCount <= 0) {
                              environment = obfuscate(environment, isAuthed, s.encrypt);
                              resolve(environment);
                              return;
                           }
                      }
...
function getAThing(o, isAuthed, callBack) {
    let selectObj = getSelectObj(o.name.replace(/s$/, ''), isAuthed),
        encryptMap = getEncryptMap(o.name.replace(/s$/, ''));
    ...
    m[o.model].find({_parentId: o.parentId}).select(selectObj).sort({name: 1}).exec((err, results) => {
        if (err) {
            ...
            if (o.submodels) {
                ...
                callBack(false, documentToObject(results, encryptMap));
            }
        }
    });
}
