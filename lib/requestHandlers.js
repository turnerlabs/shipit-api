...
e.searchEnvVars = (req, res) => {
    ...
    for (var key in req.query) {
        query.name = key + '_256';
        query.value = sha256.createHash('sha256').update(req.query[key]).digest('base64');
    }

    m.EnvVar.find({}).exec().then((data) => {
        data.forEach((envVar) => {
...
function doSearchData(data, environment) {
    ...
    if (!data[parentShipment.name]) {
        data[parentShipment.name] = {
            ...
            envVars: parentShipment.envVars.map(envVar => ({name: envVar.name, value: crypto.decrypt(envVar.value), type: envVar.type})),
...
function getEnvironment(shipmentName, name, isAuthed) {
    let parentId = '/Shipment_' + shipmentName,
        ...
        let environmentPromise = m.Environment.findOne({name: name, _parentId: parentId}).select(selectObj).exec();
